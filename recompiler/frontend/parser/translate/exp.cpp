...
bool handleExp(Builder& builder, parser::pc_t pc, parser::code_p_t* pCode) {
  ...
  auto src0 = OpSrc((eOperandKind)((OperandKind_t)eOperandKind::VGPR + inst.template get<EXP::Field::VSRC0>()));
  auto src1 = OpSrc((eOperandKind)((OperandKind_t)eOperandKind::VGPR + inst.template get<EXP::Field::VSRC1>()));
  auto src2 = OpSrc((eOperandKind)((OperandKind_t)eOperandKind::VGPR + inst.template get<EXP::Field::VSRC2>()));
  auto src3 = OpSrc((eOperandKind)((OperandKind_t)eOperandKind::VGPR + inst.template get<EXP::Field::VSRC2>()));
  ...
  *pCode += 2;
  ...
  if (target >= 0x0 && target <= 0x7) { // Attachments
    ...
    if (useExecMask) {
      builder.createInstruction(create::discardOp(OpSrc(eOperandKind::ExecLo, true, false)));
    }
