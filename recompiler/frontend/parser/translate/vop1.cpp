#include "../debug_strings.h"
#include "../instruction_builder.h"
#include "../opcodes_table.h"
#include "builder.h"
#include "encodings.h"
#include "frontend/ir_types.h"
#include "translate.h"

#include <bitset>
#include <format>
#include <stdexcept>

namespace compiler::frontend::translate {
InstructionKind_t handleVop1(parser::Context& ctx, parser::pc_t pc, parser::code_p_t* pCode, bool extended) {
  using namespace parser;

  parser::eOpcode op;
  OpDst           vdst;
  OpSrc           src0;

  create::IRBuilder ir(ctx.instructions);
  create::IRBuilder vir(ctx.instructions, true);

  if (extended) {
    auto inst = VOP3(getU64(*pCode));
    op        = (parser::eOpcode)(OPcodeStart_VOP1 + inst.template get<VOP3::Field::OP>() - OpcodeOffset_VOP1_VOP3);

    auto const vdst_ = eOperandKind::VGPR(inst.template get<VOP3::Field::VDST>());
    auto const src0_ = eOperandKind((eOperandKind_t)inst.template get<VOP3::Field::SRC0>());

    auto const           omod   = inst.template get<VOP3::Field::OMOD>();
    std::bitset<3> const negate = inst.template get<VOP3::Field::NEG>();
    std::bitset<3> const abs    = inst.template get<VOP3::Field::ABS>();
    auto const           clamp  = inst.template get<VOP3::Field::CLAMP>();

    src0 = OpSrc(src0_, negate[0], abs[0]);
    vdst = OpDst(vdst_, omod, clamp, false);
    *pCode += 1;
  } else {
    auto inst = VOP1(**pCode);
    op        = (parser::eOpcode)(OPcodeStart_VOP1 + inst.template get<VOP1::Field::OP>());

    vdst = OpDst(eOperandKind::VGPR(inst.template get<VOP1::Field::VDST>()));
    src0 = OpSrc(eOperandKind((eOperandKind_t)inst.template get<VOP1::Field::SRC0>()));
    if (src0.kind.isLiteral()) {
      *pCode += 1;
      src0 = OpSrc(ctx.instructions.createConstant(ir::ConstantValue {.value_u64 = **pCode}));
    }
  }

  *pCode += 1;

  switch (op) {
    case eOpcode::V_NOP: break; // ignore
    case eOpcode::V_MOV_B32: {
      vir.moveOp(vdst, src0, ir::OperandType::i32());
    } break;
    // case eOpcode::V_READFIRSTLANE_B32: {} break; // todo, needs lane elimination pass
    case eOpcode::V_CVT_I32_F64: {
      vir.convFPToSIOp(vdst, ir::OperandType::i32(), src0, ir::OperandType::f64());
    } break;
    case eOpcode::V_CVT_F64_I32: {
      vir.convSIToFPOp(vdst, ir::OperandType::f64(), src0, ir::OperandType::i32());
    } break;
    case eOpcode::V_CVT_F32_I32: {
      vir.convSIToFPOp(vdst, ir::OperandType::f32(), src0, ir::OperandType::i32());
    } break;
    case eOpcode::V_CVT_F32_U32: {
      vir.convUIToFPOp(vdst, ir::OperandType::f32(), src0, ir::OperandType::i32());
    } break;
    case eOpcode::V_CVT_U32_F32: {
      vir.convFPToUIOp(vdst, ir::OperandType::i32(), src0, ir::OperandType::f32());
    } break;
    case eOpcode::V_CVT_I32_F32: {
      vir.convFPToSIOp(vdst, ir::OperandType::i32(), src0, ir::OperandType::f32());
    } break;
      // case eOpcode::V_MOV_FED_B32: break; // Does not exist
    case eOpcode::V_CVT_F16_F32: {
      vir.packHalf2x16Op(vdst, src0, OpSrc(eOperandKind::createImm(0)));
    } break;
    case eOpcode::V_CVT_F32_F16: {
      vir.unpackHalf2x16(vdst, OpDst(eOperandKind::Unset()), src0);
    } break;
    case eOpcode::V_CVT_RPI_I32_F32: {
      vir.addFOp(OpDst(vdst.kind), src0, OpSrc(eOperandKind((eOperandKind_t)eOperandKind::eBase::ConstFloat_0_5)), ir::OperandType::f32());
      vir.floorOp(OpDst(vdst.kind), OpSrc(vdst.kind), ir::OperandType::f32());
      vir.convFPToSIOp(vdst, ir::OperandType::i32(), OpSrc(vdst.kind), ir::OperandType::f32());
    } break;
    case eOpcode::V_CVT_FLR_I32_F32: {
      vir.floorOp(vdst, src0, ir::OperandType::f32());
      vir.convFPToSIOp(vdst, ir::OperandType::i32(), OpSrc(vdst.kind), ir::OperandType::f32());
    } break;
    case eOpcode::V_CVT_OFF_F32_I4: {
      vir.convSI4ToFloat(vdst, OpSrc(src0));
    } break;
    case eOpcode::V_CVT_F32_F64: {
      vir.truncFOp(vdst, src0);
    } break;
    case eOpcode::V_CVT_F64_F32: {
      vir.extFOp(vdst, src0);
    } break;
    case eOpcode::V_CVT_F32_UBYTE0: {
      vir.bitUIExtractOp(vdst, src0, OpSrc(eOperandKind::createImm(0)), OpSrc(eOperandKind::createImm(8)), ir::OperandType::i32());
      vir.convUIToFPOp(vdst, ir::OperandType::f32(), OpSrc(vdst.kind), ir::OperandType::i32());
    } break;
    case eOpcode::V_CVT_F32_UBYTE1: {
      vir.bitUIExtractOp(vdst, OpSrc(src0), OpSrc(eOperandKind::createImm(8)), OpSrc(eOperandKind::createImm(8)), ir::OperandType::i32());
      vir.convUIToFPOp(vdst, ir::OperandType::f32(), OpSrc(vdst.kind), ir::OperandType::i32());
    } break;
    case eOpcode::V_CVT_F32_UBYTE2: {
      vir.bitUIExtractOp(vdst, OpSrc(src0), OpSrc(eOperandKind::createImm(16)), OpSrc(eOperandKind::createImm(8)), ir::OperandType::i32());
      vir.convUIToFPOp(vdst, ir::OperandType::f32(), OpSrc(vdst.kind), ir::OperandType::i32());
    } break;
    case eOpcode::V_CVT_F32_UBYTE3: {
      vir.bitUIExtractOp(vdst, OpSrc(src0), OpSrc(eOperandKind::createImm(24)), OpSrc(eOperandKind::createImm(8)), ir::OperandType::i32());
      vir.convUIToFPOp(vdst, ir::OperandType::f32(), OpSrc(vdst.kind), ir::OperandType::i32());
    } break;
    case eOpcode::V_CVT_U32_F64: {
      vir.convFPToUIOp(vdst, ir::OperandType::i32(), src0, ir::OperandType::f64());
    } break;
    case eOpcode::V_CVT_F64_U32: {
      vir.convUIToFPOp(vdst, ir::OperandType::f64(), src0, ir::OperandType::i32());
    } break;
    case eOpcode::V_TRUNC_F64: {
      vir.truncOp(vdst, src0, ir::OperandType::f64());
    } break;
    case eOpcode::V_CEIL_F64: {
      vir.ceilOp(vdst, src0, ir::OperandType::f64());
    } break;
    case eOpcode::V_RNDNE_F64: {
      vir.roundEvenOp(vdst, src0, ir::OperandType::f64());
    } break;
    case eOpcode::V_FLOOR_F64: {
      vir.floorOp(vdst, src0, ir::OperandType::f64());
    } break;
    case eOpcode::V_FRACT_F32: {
      vir.fractOp(vdst, src0, ir::OperandType::f32());
    } break;
    case eOpcode::V_TRUNC_F32: {
      vir.truncOp(vdst, src0, ir::OperandType::f32());
    } break;
    case eOpcode::V_CEIL_F32: {
      vir.ceilOp(vdst, src0, ir::OperandType::f32());
    } break;
    case eOpcode::V_RNDNE_F32: {
      vir.roundEvenOp(vdst, src0, ir::OperandType::f32());
    } break;
    case eOpcode::V_FLOOR_F32: {
      vir.floorOp(vdst, src0, ir::OperandType::f32());
    } break;
    case eOpcode::V_EXP_F32: {
      vir.exp2Op(vdst, src0);
    } break;
    case eOpcode::V_LOG_CLAMP_F32: {
      vir.log2Op(OpDst(vdst.kind), src0);
      vir.clampFMinMaxOp(vdst, OpSrc(vdst.kind), ir::OperandType::f32());
    } break;
    case eOpcode::V_LOG_F32: {
      vir.log2Op(vdst, src0);
    } break;
    case eOpcode::V_RCP_CLAMP_F32: {
      vir.rcpOp(OpDst(vdst.kind), src0, ir::OperandType::f32());
      vir.clampFMinMaxOp(vdst, OpSrc(vdst.kind), ir::OperandType::f32());
    } break;
    case eOpcode::V_RCP_LEGACY_F32: {
      vir.rcpOp(OpDst(vdst.kind), src0, ir::OperandType::f32());
      vir.clampFZeroOp(vdst, OpSrc(vdst.kind), ir::OperandType::f32());
    } break;
    case eOpcode::V_RCP_F32: {
      vir.rcpOp(vdst, src0, ir::OperandType::f32());
    } break;
    case eOpcode::V_RCP_IFLAG_F32: { // todo exception?
      vir.rcpOp(vdst, src0, ir::OperandType::f32());
    } break;
    case eOpcode::V_RSQ_CLAMP_F32: {
      vir.rsqrtOp(OpDst(vdst.kind), src0, ir::OperandType::f32());
      vir.clampFMinMaxOp(vdst, OpSrc(vdst.kind), ir::OperandType::f32());
    } break;
    case eOpcode::V_RSQ_LEGACY_F32: {
      vir.rsqrtOp(OpDst(vdst.kind), src0, ir::OperandType::f32());
      vir.clampFZeroOp(vdst, OpSrc(vdst.kind), ir::OperandType::f32());
    } break;
    case eOpcode::V_RSQ_F32: {
      vir.rsqrtOp(vdst, src0, ir::OperandType::f32());
    } break;
    case eOpcode::V_RCP_F64: {
      vir.rcpOp(vdst, src0, ir::OperandType::f64());
    } break;
    case eOpcode::V_RCP_CLAMP_F64: {
      vir.rcpOp(OpDst(vdst.kind), src0, ir::OperandType::f64());
      vir.clampFMinMaxOp(vdst, OpSrc(vdst.kind), ir::OperandType::f64());
    } break;
    case eOpcode::V_RSQ_F64: {
      vir.rsqrtOp(vdst, src0, ir::OperandType::f64());
    } break;
    case eOpcode::V_RSQ_CLAMP_F64: {
      vir.rsqrtOp(OpDst(vdst.kind), src0, ir::OperandType::f64());
      vir.clampFMinMaxOp(vdst, OpSrc(vdst.kind), ir::OperandType::f64());
    } break;
    case eOpcode::V_SQRT_F32: {
      vir.sqrtOp(vdst, src0, ir::OperandType::f32());
    } break;
    case eOpcode::V_SQRT_F64: {
      vir.sqrtOp(vdst, src0, ir::OperandType::f64());
    } break;
    case eOpcode::V_SIN_F32: {
      vir.sinOp(vdst, src0);
    } break;
    case eOpcode::V_COS_F32: {
      vir.cosOp(vdst, src0);
    } break;
    case eOpcode::V_NOT_B32: {
      vir.moveOp(vdst, OpSrc(src0.kind, !src0.flags.getNegate(), false), ir::OperandType::i32());
    } break;
    case eOpcode::V_BFREV_B32: {
      vir.bitReverseOp(vdst, src0, ir::OperandType::i32());
    } break;
    case eOpcode::V_FFBH_U32: {
      // relative to msb
      vir.bitReverseOp(OpDst(vdst.kind), src0, ir::OperandType::i32());
      vir.findILsbOp(vdst, OpSrc(vdst.kind), ir::OperandType::i32());
    } break;
    case eOpcode::V_FFBL_B32: {
      vir.findILsbOp(vdst, src0, ir::OperandType::i32());
    } break;
    // case eOpcode::V_FFBH_I32: {} break; // todo
    case eOpcode::V_FREXP_EXP_I32_F64: {
      vir.frexpOp(vdst, OpDst(eOperandKind::Unset()), src0, ir::OperandType::f64());
    } break;
    case eOpcode::V_FREXP_MANT_F64: {
      vir.frexpOp(OpDst(eOperandKind::Unset()), vdst, src0, ir::OperandType::f64());
    } break;
    case eOpcode::V_FRACT_F64: {
      vir.fractOp(vdst, src0, ir::OperandType::f64());
    } break;
    case eOpcode::V_FREXP_EXP_I32_F32: {
      // amdllpc translates frexp to
      // v_frexp_exp_i32_f32_e32 v1, v0
      // v_frexp_mant_f32_e32 v0, v0
      vir.frexpOp(vdst, OpDst(eOperandKind::Unset()), src0, ir::OperandType::f32());
    } break;
    case eOpcode::V_FREXP_MANT_F32: {
      vir.frexpOp(OpDst(eOperandKind::Unset()), vdst, src0, ir::OperandType::f32());
    } break;
    // case eOpcode::V_CLREXCP: {} break; // does not exist
    // case eOpcode::V_MOVRELD_B32: {} break; // todo
    // case eOpcode::V_MOVRELS_B32: {} break; //todo
    // case eOpcode::V_MOVRELSD_B32: {} break;// todo
    // case eOpcode::V_LOG_LEGACY_F32: {} break; // does not exist
    // case eOpcode::V_EXP_LEGACY_F32: {} break; // does not exist
    default: throw std::runtime_error(std::format("missing inst {}", debug::getDebug(op))); break;
  }
  return conv(op);
}
} // namespace compiler::frontend::translate