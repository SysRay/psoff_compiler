...
InstructionKind_t parseInstruction(Context& ctx, pc_t pc, code_p_t* pCode) {
  ...
  switch (getEncoding(**pCode)) {
    ...
    case eEncoding::VOP3: {
      ...
      if (op >= OpcodeOffset_VOP1_VOP3) return handleVop1(ctx, pc, pCode, true);
      if (op >= 0x140) return handleVop3(ctx, pc, pCode);
      if (op >= OpcodeOffset_VOP2_VOP3) return handleVop2(ctx, pc, pCode, true);
