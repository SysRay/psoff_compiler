...
#define __INST(kind, flags, dstOps, srcOps) makeInstDef_(eInstKind::kind, {eInstructionFlags::flags}, std::array {dstOps}, std::array {srcOps}, #kind)

#define __INST_NO_OPS(kind, flags)                                                                                                                             \
  makeInstDef_(eInstKind::kind, {eInstructionFlags::flags}, std::array<OutputOperand, 0> {}, std::array<InputOperand, 0> {}, #kind)
...
#define __INST_NO_SRC(kind, flags, dstOps) makeInstDef_(eInstKind::kind, {eInstructionFlags::flags}, std::array {dstOps}, std::array<InputOperand, 0> {}, #kind)

static constexpr std::array kInstTable = {
    __INST_NO_DST(YieldOp, kTerminator, __OPS(i::i64)),
};
