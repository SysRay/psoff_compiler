...
bool createRegions(Builder& builder, mlir::func::FuncOp& func, RegionBuilder const& regions, std::span<pcmapping_t> const& mapping) {
  regions.for_each([&](uint32_t start, uint32_t end, void* region) {
    for (auto n = start; n < end; ++n) {
      auto it = std::find_if(mapping.begin(), mapping.end(), [n](auto const& item) { return item.first == n; });
      if (it == mapping.end()) {
  ...
  //   // Check if the operation is a psoff.branch or psoff.cbranch
  //   if (auto branchOp = dyn_cast<mlir::psoff::Branch>(op)) {
  //     // TODO: Process psoff.branch
  //     // Example: std::cout << "Found psoff.branch at: " << op->getLoc() << "\n";
  //   } else if (auto cbranchOp = dyn_cast<mlir::psoff::CBranch>(op)) {
  //     // TODO: Process psoff.cbranch
  //     // Example: std::cout << "Found psoff.cbranch at: " << op->getLoc() << "\n";
  //   } else if (auto cbranchOp = dyn_cast<mlir::func::ReturnOp>(op)) {
  //     regions.addReturn(n);
  //   }
  // });
